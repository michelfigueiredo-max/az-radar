    ...
    <div style={{ padding: mobile ? "14px 12px" : "22px 24px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      {mobile ? (
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: `90px 1fr 120px 90px 60px 90px${isMaster ? " 80px 75px" : ""}`, padding: "9px 16px", background: "#F1F5FB", borderBottom: `1px solid ${C.border}`, fontSize: 10, color: C.textMuted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", gap: 8 }}>
            <span>Data</span><span>Nome / Doc</span><span>Cliente</span><span>Perfil</span><span>Itens</span><span>Valor</span>
          ...
          {filtered.map((h, i) => {
            ...
            return (
              <div key={h.id} style={{ display: "grid", gridTemplateColumns: `90px 1fr 120px 90px 60px 90px${isMaster ? " 80px 75px" : ""}`, padding: "11px 16px", borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : "none", background: i % 2 === 0 ? C.surface : "#F8FAFC", gap: 8, alignItems: "center" }}>
    ...
    <div style={{ padding: mobile ? "14px 12px" : "22px 24px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", marginBottom: 14 }}>
        {mobile ? (
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 70px 85px 80px 80px 65px", padding: "9px 16px", background: "#F1F5FB", borderBottom: `1px solid ${C.border}`, fontSize: 10, color: C.textMuted, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", gap: 8 }}>
              <span>Cliente</span><span>Consul.</span><span>Receita</span><span>Custo API</span><span>Margem</span><span>%</span>
            </div>
            {porCliente.map((c, i) => {
              const m = c.cob - c.api; const pct = c.cob > 0 ? Math.round(m / c.cob * 100) : 0;
              return (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 70px 85px 80px 80px 65px", padding: "12px 16px", borderBottom: i < porCliente.length - 1 ? `1px solid ${C.border}` : "none", background: i % 2 === 0 ? C.surface : "#F8FAFC", gap: 8, alignItems: "center" }}>
...
const VIEW_TABS = ["dossie", "historico", "master"];

export default function App() {
  const mobile = useIsMobile();
  const [isMaster, setIsMaster] = useState(false);
  const [view, setView] = useState("dossie");
        ...
        <div style={{ display: "flex", gap: 2, overflowX: "auto" }}>
          {(isMaster ? VIEW_TABS : VIEW_TABS.filter(v => v !== "master")).map(v => (
            <button key={v} onClick={() => setView(v)} style={{ padding: mobile ? "5px 9px" : "6px 13px", borderRadius: 7, border: "none", cursor: "pointer", fontSize: mobile ? 11 : 12, fontWeight: 600, whiteSpace: "nowrap", background: view === v ? "rgba(255,255,255,0.18)" : "transparent", color: view === v ? "#fff" : "rgba(255,255,255,0.48)", transition: "all 0.15s" }}>
              {v === "dossie" ? "📋 Relatório" : v === "historico" ? "🔍 Histórico" : "📊 Master"}
            </button>
          ))}
        </div>
